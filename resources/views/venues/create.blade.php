...
@section('title', 'レンタルスペース・貸し会議室を投稿する - ' . config('app.name'))
@section('description', '地図をタップして場所を選び、レンタルスペース・貸し会議室の名称・住所などを投稿できます。ログイン不要・匿名で投稿可能です。')

@section('content')
<div class="container my-4">
  <h1 class="h4 mb-3">➕ スペース・会議室を投稿する</h1>
  ...
  <form method="POST" action="{{ route('venues.store') }}" class="bg-light p-3 rounded shadow-sm">
    ...
    <div class="row mb-3">
      <div class="col-6">
        <label class="form-label">収容人数（任意）</label>
        <input type="number" name="capacity" class="form-control" min="1">
      </div>
      <div class="col-6">
        <label class="form-label">目安料金（円/時間・任意）</label>
    ...
    <button type="submit" class="btn btn-space w-100">登録する</button>
  </form>
</div>
@endsection
